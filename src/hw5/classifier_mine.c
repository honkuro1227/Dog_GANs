...
matrix backward_layer(layer *l, matrix delta)
{
    ...
    matrix dw = matrix_mult_matrix(delta,xt); // replace this
    l->dw = dw;
    free_matrix(xt);
    free_matrix(dw);
    // 1.4.3
...
void update_layer(layer *l, double rate, double momentum, double decay)
{
    ...
    // save it to l->v
    matrix deltaw  = l->dw;
    matrix decayw  = deltaw;
    scale_matrix(decayw,decay);
    matrix momentumw=deltaw;
    scale_matrix(momentumw,momentum);
    l->v =axpy_matrix(1,matrix_sub_matrix(deltaw,decayw),momentumw);

    // Update l->w
    matrix v=l->v;
    scale_matrix(v,rate);
    l->w=axpy_matrix(1,l->v,v);
    // Remember to free any intermediate results to avoid memory leaks
    free_matrix(decayw);
    free_matrix(momentumw);
    free_matrix(deltaw);
}
