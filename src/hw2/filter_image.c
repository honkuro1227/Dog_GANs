...
image cc_images(image edge, image ori){
    image re = make_image(2*edge.w, edge.h, 3);
    for(int x=0;x<edge.w;x++){
        for(int y=0;y<edge.h;y++){
            set_pixel(re,x,y,0,get_pixel(edge,x,y,0));
            set_pixel(re,x,y,1,get_pixel(edge,x,y,0));
            set_pixel(re,x,y,2,get_pixel(edge,x,y,0));
        }
    }
    for(int x=edge.w-1;x<re.w;x++){
        for(int y=0;y<edge.h;y++){
            set_pixel(re,x,y,0,get_pixel(ori,x-edge.w+1,y,0));
            set_pixel(re,x,y,1,get_pixel(ori,x-edge.w+1,y,1));
            set_pixel(re,x,y,2,get_pixel(ori,x-edge.w+1,y,2));
        }
    }
    return  re;
}
image colorize_sobel(image im)
{
    ...
    image re = make_image(2*im.w, im.h, 3);
    for(int x=0;x<im.w;x++){
        for(int y=0;y<im.h;y++){
            set_pixel(re,x,y,0,get_pixel(s[0],x,y,0));
            set_pixel(re,x,y,1,get_pixel(s[0],x,y,0));
            set_pixel(re,x,y,2,get_pixel(s[0],x,y,0));
        }
    }
    for(int x=im.w-1;x<re.w;x++){
        for(int y=0;y<im.h;y++){
            set_pixel(re,x,y,0,get_pixel(im,x-im.w+1,y,0));
            set_pixel(re,x,y,1,get_pixel(im,x-im.w+1,y,1));
            set_pixel(re,x,y,2,get_pixel(im,x-im.w+1,y,2));
        }
    }
